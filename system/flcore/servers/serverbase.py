...
class Server(object):
    ...
    def aggregate_param_diff(self):
        ...
        average_updated_params = [
            sum(element) / len(topk_updated_params)
            for element in zip(*topk_updated_params)
        ]

        for server_param, w, updated_params in zip(
            self.global_model.parameters(),
            self.uploaded_weights,
            average_updated_params,
        ):
            server_param.data += updated_params * w

    def get_top_k(self, aggregated_params, topk_algo):
        if topk_algo == "global":
            topk_ = [self.global_topk(grad) for grad in aggregated_params]
        elif topk_algo == "chunk":
            topk_ = [self.chunk_topk(grad) for grad in aggregated_params]
        else:
            topk_ = aggregated_params
    ...
    def chunk_topk(self, gradient):
        ...
        for chunk in chunks:
            ...
            chunk.data = torch.where(
                torch.abs(chunk.data) == local_max, chunk.data, 0.0
            )
    ...
    def global_topk(self, gradient):
        ...
        for g in gradient:
            g.data = torch.where(torch.abs(g.data) >= min_, g.data, 0.0)

        return gradient

    def get_min_grad(self, gradient):

        all_grads = torch.cat([grad for grad in gradient])
